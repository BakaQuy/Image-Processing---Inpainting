function [offsets] = propOdd(imgA, imgB, patchSize, offsets, yMaxSize, xMaxSize, mask)

for y=1:size(offsets,1)
    for x=1:size(offsets,2)
            
        bPatch = imgB(y:y+patchSize-1,x:x+patchSize-1,:);
            
        offset=offsets(y,x,:);
        
        up = offsets(max(1,y-1),x,:);
        left = offsets(y,max(1,x-1),:);

        upOffset = [min(max(1,up(1)+1),yMaxSize),up(2)];
        leftOffset = [left(1),min(max(1,left(2)+1),xMaxSize)]; 
