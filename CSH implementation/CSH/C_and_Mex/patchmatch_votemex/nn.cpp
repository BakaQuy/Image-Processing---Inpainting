...
#pragma omp parallel for schedule(static, 4)
  for (int y = box.ymin; y < box.ymax; y++) {
    ...
    for (int x = box.xmin; x < box.xmax; x++) {
      ...
      //if (x == 1 && y == 1) { mexPrintf("1, 1 => %d, %d (%d)\n", xp, yp, row[x]); }
    }
  }
  return ans;
}
...
BITMAP *weighted_vote_n(Params *p, BITMAP *a, BITMAP *b, double *accum,
							 BITMAP *ann, BITMAP *bnn,
							 BITMAP *bmask, BITMAP *bweight,
							 double coherence_weight, double complete_weight,         
							 RegionMasks *amask, BITMAP *aweight, BITMAP *ainit, double *flattenedKernels,
                                                                                     RegionMasks *region_masks, BITMAP *aconstraint, int mask_self_only, int factor, int cutoff) 
{
          f1 = fopen("csh_data.txt", "wt");
          mexPrintf("vote, mask_self_only=%d\n", mask_self_only);
          mexPrintf("SIMON: in nn.cpp:: weighted_vote_n\n");
          
//           int ind = 0;
//           for (int i = 0 ; i < PATCH_W*PATCH_W ; i++) {
//                     mexPrintf("-- weighted_vote_n: ------ flattenedKernels row %d -----------\n", i);
//                     for (int j = 0; j < PATCH_W*PATCH_W ; j++)
//                               mexPrintf("%f ", flattenedKernels[ind++]);
//           }
          
//           int sz = ann->w * ann->h; sz = sz << 2; // 4 * w * h
//           accum = new double[sz]; // accumulator for RGB and total weights
//           memset((void *) accum, 0, sizeof(double) * sz);
          mexPrintf("$$$$$$$$$$ weighted_vote_n  ADDR is: %p\n",accum);
          
          double *distanceWeights = new double[PATCH_W*PATCH_W]; // will store all width^2 'distanceWeights' between 2 patches
          memset((void *) distanceWeights, 0, sizeof(double) * PATCH_W*PATCH_W);
          ...
          /* Coherence */
          mexPrintf("weighted_vote_n, amask: %p, bmask: %p, aweight: %p, bweight: %p, mask_self_only: %d\n", amask, bmask, aweight, bweight, mask_self_only);
          
          mexPrintf("weighted_vote_n  FACTOR = %d\n", factor);
          
          for (int ay = box.ymin; ay < box.ymax; ay++) {
                    int *amask_row = amask ? (int *) amask->bmp->line[ay]: NULL;
                    for (int ax = box.xmin; ax < box.xmax; ax++) {
                              if (amask && amask_row[ax]) { continue; } // this row of the A-patch is masked-out
                              int bx, by;
                              ...
                              // create a width-by-width matrix with the appropriate flattenedKernels-weighted distances between the patches
                              memset((void *) distanceWeights, 0, sizeof(double) * PATCH_W*PATCH_W);
                              int print = 0;
                              if (ay == 147 && ax == 300) // print what happens at a certain block
                                        print = 1;
                              
                              int sumDist2 = weighted_fast_patch_dist(a, ax, ay, b, bx, by, PATCH_W, flattenedKernels, distanceWeights, factor, print,cutoff);                              
//
//                       double w = wa;
                              // this (w) is a weight per patch of A (e.g. could be the ann error)
//                       if (aweight) { w *= ((float *) aweight->line[ay+PATCH_W/2])[ax+PATCH_W/2]; /*mexPrintf("aweight: %f\n", w);*/ } // weight is centered, unlike NN fields
//                       if (!mask_self_only && bweight) { w *= ((float *) bweight->line[by+PATCH_W/2])[bx+PATCH_W/2]; }
                              //if (w < 1e-30) { w = 1e-20; }
                              if (print){ // print what happens at a certain block
                                        mexPrintf("\n==========================================================\n");
                                        mexPrintf("\n============= in  weighted_vote_n ===========================\n");
                                        mexPrintf("\n==========================================================\n");
                              }
                              int distInd = 0;
                              int loc = 200*ann->w + 200;
                              for (int dy = 0; dy < PATCH_W; dy++) {
                                        int *brow = ((int *) b->line[by+dy]) + bx;
                                        double *prow = &accum[4*((ay+dy)*ann->w+ax)];
                                        for (int dx = 0; dx < PATCH_W; dx++) {
                                                  double w = distanceWeights[distInd++];
                                                  int c = brow[dx];                         // b pixel
                                                  double *p = &prow[4*dx];       // current accumulated value
                                                  p[0] += (c&255)*w;                        // R
                                                  p[1] += ((c>>8)&255)*w;             // G
                                                  p[2] += (c>>16)*w;                        // B
                                                  p[3] += w;                                         // weight
//                                           if ((ay+dy)*ann->w+ax + dx==loc) // print what happens at a certain pixel
//                                                     mexPrintf("ay: %d, ax: %d, dy: %d, dx: %d, distInd: %d, w: %f, bPix: (%d,%d,%d), p: (%f,%f,%f,%f) \n",
//                                                                         ay,ax,dy, dx, distInd, w, c&255, (c>>8)&255, c>>16, p[0], p[1], p[2], p[3]);
                                                  if (print) // print what happens at a certain block
                                                  {
                                                            mexPrintf("patchDist: %d, dy: %d, dx: %d, distInd: %d, w: %f, bPix: (%d,%d,%d), p: (%f,%f,%f,%f) \n",
                                                                    sumDist2, dy, dx, distInd, w, c&255, (c>>8)&255, c>>16, p[0], p[1], p[2], p[3]);
                                                  }
                                                  int targetX = ax+dx, targetY = ay + dy;
                                                  if((targetY >= PATCH_W-1) && (targetY <= ann->h-PATCH_W) && (targetX >= PATCH_W-1) && (targetX <= ann->w-PATCH_W))
                                                  {
                                                            int pixelInd = (ay+dy)+(ax+dx)*ann->h; // Matlab ordering
                                                            fprintf(f1, "%d %d %d   %d %d   %d %d %d   %f\n", pixelInd, ax+dx, ay+dy,  bx+dx, by+dy, c&255, (c>>8)&255, c>>16,  w);
//                                                             fprintf(f1, "%d %d %d   %d %d   %d %d %d   %f   %d %d\n", pixelInd, ax+dx, ay+dy,  bx+dx, by+dy, c&255, (c>>8)&255, c>>16,  w,  dx, dy);
                                                  }
                                        }
                              }
                              if (print){ // print what happens at a certain block
                                        mexPrintf("\n==========================================================\n");
                                        mexPrintf("\n============= end of  weighted_vote_n ========================\n");
                                        mexPrintf("\n==========================================================\n");
                              }
...
template<int PATCH_W>
BITMAP *weighted_KNN_dispersion_vote_n(Params *p, BITMAP *a, BITMAP *b, double *accum,
							 BITMAP **a_knn, BITMAP **b_knn,
							 BITMAP *bmask, BITMAP *bweight,
							 double coherence_weight, double complete_weight,         
							 RegionMasks *amask, BITMAP *aweight, BITMAP *ainit, double *flattenedKernels,
							 RegionMasks *region_masks, BITMAP *aconstraint, int mask_self_only, int factor, int cutoff , int K_of_KNN) 
{
          mexPrintf("GUY: in nn.cpp:: weighted_KNN_dispersion_vote_n, K_of_KNN = %d\n" , K_of_KNN);
          mexPrintf("$$$$$$$$$$ weighted_vote_n  ADDR is: %p\n",accum);
          
          double *distanceWeights = new double[PATCH_W*PATCH_W]; // will store all width^2 'distanceWeights' between 2 patches
          memset((void *) distanceWeights, 0, sizeof(double) * PATCH_W*PATCH_W);
          
		 // a_knn and b_knn are array of bitmap pointers

          double wa = 1, wb = 1;
          if (b_knn) {
                wa = coherence_weight / ((a_knn[0]->h-PATCH_W+1) * (a_knn[0]->w-PATCH_W+1));
				wb = complete_weight  / ((b_knn[0]->h-PATCH_W+1) * (b_knn[0]->w-PATCH_W+1));
          }
          
          Box box = get_abox(p, a_knn[0], amask);
          
          /* Coherence */
          mexPrintf("weighted_vote_n, amask: %p, bmask: %p, aweight: %p, bweight: %p, mask_self_only: %d\n", amask, bmask, aweight, bweight, mask_self_only);
          
          mexPrintf("weighted_vote_n  FACTOR = %d\n", factor);
          for (int k = 0 ; k < K_of_KNN ; k++)
		  {
			  for (int ay = box.ymin; ay < box.ymax; ay++) 
			  {
					int *amask_row = amask ? (int *) amask->bmp->line[ay]: NULL;
					for (int ax = box.xmin; ax < box.xmax; ax++) 
					{
						  ...
						  for (int dy = 0; dy < PATCH_W; dy++) {
								int *brow = ((int *) b->line[by+dy]) + bx;
								double *prow = &accum[4*((ay+dy)*a_knn[k]->w+ax)];
								for (int dx = 0; dx < PATCH_W; dx++) {
										  double w = distanceWeights[distInd++];
										  int c = brow[dx];                         // b pixel
										  double *p = &prow[4*dx];       // current accumulated value
		  ...
		  if (b_knn) {
				mexPrintf("Using completeness\n");
				for (int k = 0 ; k < K_of_KNN ; k++) {
					for (int by = 0; by < b_knn[k]->h-PATCH_W+1; by++) {
						  for (int bx = 0; bx < b_knn[k]->w-PATCH_W+1; bx++) {
							if (bmask && ((int *) bmask->line[by])[bx]) { continue; }
							int ax, ay;
							getnn(b_knn[k], bx, by, ax, ay);
										// Note: currently an input patch CAN vote to an output patch outside the mask 'amask' (not sure this is good)
										//if (amask && ((int *) amask->line[ay])[ax]) { continue; }
							double w = wb;
							if (bweight) { w *= ((float *) bweight->line[by+PATCH_W/2])[bx+PATCH_W/2]; }
          ...
          BITMAP *ans = NULL;
          mexPrintf("ainit: %p\n", ainit);
          ans = ainit ? norm_image_init(accum, a_knn[0]->w, a_knn[0]->h, ainit) : norm_image(accum, a_knn[0]->w, a_knn[0]->h);
...
//////// Suppports K_ANN only
template<int PATCH_W>
BITMAP *weighted_KNN_vote_n(Params *p, BITMAP *a, BITMAP *b, double *accum, BITMAP **a_knn,
							BITMAP *bmask, BITMAP *bweight,double coherence_weight, double complete_weight, RegionMasks *amask, BITMAP *aweight, 
							BITMAP *ainit, double *flattenedKernel, RegionMasks *region_masks, BITMAP *aconstraint, int mask_self_only, int factor, int cutoff, int K_of_KNN)
{
          int left, right, top, bottom , k;

		  mexPrintf("Guy: in nn.cpp:: weighted_KNN_vote_n\n");

		  //int CenterX = (PATCH_W/2) - 1; // patch width assumed to be even!!!
		  int CenterY = (PATCH_W/2) - 1; // patch width assumed to be even!!!
         int  CenterX = CenterY;
          mexPrintf("$$$$$$$$$$ weighted_KNN_vote_n ADDR is: %p\n",accum);
		  ...
		  for (int ay = box.ymin; ay < box.ymax; ay++) 
		  {
                ...
                for (int ax = box.xmin; ax < box.xmax; ax++) 
				{
					  for (int k = 0 ; k < K_of_KNN ; k++)
					  {
						  ...
						  // Note: currently an output patch CANNOT vote to an input patch outside the mask 'bmask' (this seems reasonable)
						  if (!mask_self_only && bmask && ((int *) bmask->line[by])[bx]) { continue; }
						  double *p = &accum[4*((ay+CenterY)*(a_knn[k]->w)+(ax+CenterX))]; // target pixel (center of patch)
						  // find the weight between the two patches
						  double distanceWeight;
						  int sumDist2 = gaussian_fast_patch_dist(a, ax, ay, b, bx, by, PATCH_W, flattenedKernel, distanceWeight, factor, print,cutoff);
						  // add the central pixel with the calculated weight
						  int *brow = ((int *) b->line[by+CenterY]);
						  int c = brow[bx + CenterX];                  // b pixel
						  p[0] += (c&255)*distanceWeight;              // R
						  p[1] += ((c>>8)&255)*distanceWeight;         // G
						  p[2] += (c>>16)*distanceWeight;              // B
						  p[3] += distanceWeight;                      // weight
					  } // k loop
                } // ax loop
		   } // ay loop
		  ...
		  BITMAP *ans = NULL;
          mexPrintf("ainit: %p\n", ainit);
          ans = ainit ? norm_image_init(accum, a->w, a->h, ainit) : norm_image(accum, a->w, a->h);
          mexPrintf("after norm_image_init\n");
...
////////
template<int PATCH_W>
BITMAP *single_local_vote_n(Params *p, BITMAP *a, BITMAP *b, double *accum,
        BITMAP *bmask, BITMAP *bweight,
        double coherence_weight, double complete_weight,
        RegionMasks *amask, BITMAP *aweight, BITMAP *ainit, double *flattenedKernel,
        RegionMasks *region_masks, BITMAP *aconstraint, int mask_self_only, int factor, int cutoff, int radius) 
{
          
          int left, right, top, bottom;
//           mexPrintf("vote, mask_self_only=%d\n", mask_self_only);
          mexPrintf("SIMON: in nn.cpp:: single_local_vote_n\n");
          
          int ind = 0;
#ifdef _ADD_NLM_DEBUG_INFO
          for (int i = 0 ; i < 1; i++) {
                    mexPrintf("\n-- single_local_vote_n: (flattenedKernel)------ row %d -----------\n", i);
                    for (int j = 0; j < PATCH_W*PATCH_W ; j++)
                              mexPrintf("%f ", flattenedKernel[ind++]);
          }
          mexPrintf("\n");
#endif
          int halfPW = (PATCH_W-1)/2; // patch width assumed to be odd!!!

		  mexPrintf("$$$$$$$$$$ single_local_vote_n ADDR is: %p\n",accum);
          ...
          /* Coherence */
#ifdef _ADD_NLM_DEBUG_INFO
          mexPrintf("single_local_vote_n, amask: %p, bmask: %p, aweight: %p, bweight: %p, mask_self_only: %d\n", amask, bmask, aweight, bweight, mask_self_only);
          
          mexPrintf("single_local_vote_n  FACTOR = %d\n", factor);
#endif
          for (int ay = box.ymin; ay < box.ymax; ay++) {
#ifdef _ADD_NLM_DEBUG_INFO
				    if ((ay%20) == 1)
                    ...
                    for (int ax = box.xmin; ax < box.xmax; ax++) {
                              ...
                              int bx, by;                              
#ifdef _ADD_NLM_DEBUG_INFO
                              if ((ay == 147 && ax == 300)) // || (ay == 250 && ax == 210)) // print what happens at a certain block
                                        print = 1;
#endif                              
                              double *p = &accum[4*((ay+halfPW)*(a->w)+(ax+halfPW))]; // target pixel (center of patch)

                              left = max(0, ax + halfPW - radius);
                                                                                             //            0        3                      10                     7               1
                              right = min(a->w - PATCH_W, ax + halfPW + radius - PATCH_W + 1);
                              top = max(0, ay + halfPW - radius);
                              bottom = min(a->h- PATCH_W, ay + halfPW + radius - PATCH_W + 1);
#ifdef _ADD_NLM_DEBUG_INFO
							  if (print) // print what happens at a certain block
                              {
                                        mexPrintf("\n======================================================\n");
                                        mexPrintf("============== STATUS at 'print' location ==================\n");
                                        mexPrintf("======================================================\n");
                                        mexPrintf("a->w:%d,a->h:%d,PATCH_W:%d,halfPW:%d,radius:%d\n",
                                                a->w, a->h, PATCH_W, halfPW, radius);
                                        mexPrintf("ax:%d,ay:%d,left:%d,right:%d,top:%d,bottom:%d,ax+halfPW:%d,ay+halfPW:%d\n",
                                                ax, ay, left, right, top, bottom, ax+halfPW, ay+halfPW);
                                        mexPrintf("\n======================================================\n");
                                        mexPrintf("============== now enumerating the BB (lrtb) ==================\n");
                                        mexPrintf("======================================================\n");
                              }
#endif
							  // enumerate local (2*radius+1)^2 neighborhood
                              for (int bx = left; bx <=right; bx++) {
                                        for (int by = top; by <= bottom; by++) {
                                                  ...
                                                  int sumDist2 = gaussian_fast_patch_dist(a, ax, ay, b, bx, by, PATCH_W, flattenedKernel, distanceWeight, factor, print,cutoff);
                                                  // add the central pixel with the calculated weight
                                                  int *brow = ((int *) b->line[by+halfPW]);
                                                  int c = brow[bx + halfPW];                         // b pixel
                                                  p[0] += (c&255)*distanceWeight;                        // R
                                                  p[1] += ((c>>8)&255)*distanceWeight;             // G
                                                  p[2] += (c>>16)*distanceWeight;                        // B
                                                  p[3] += distanceWeight;                                         // weight
#ifdef _ADD_NLM_DEBUG_INFO
                                                  if (print) // print what happens at a certain block
                                                            mexPrintf("bx: %d, by: %d, patchDist: %d, distanceWeight: %f, bPix: (%d,%d,%d), p: (%f,%f,%f,%f) \n",
																	   bx, by, sumDist2, distanceWeight, c&255, (c>>8)&255, c>>16, p[0], p[1], p[2], p[3]);
#endif
                                        } // by loop
                              } // bx loop
          ...
          double *accumMid = &accum[4*((147+halfPW)*(a->w)+(300+halfPW))];
#ifdef _ADD_NLM_DEBUG_INFO
          mexPrintf("TESTING in single_local_vote_n: accumMid: %f, %f, %f, %f, %f, %f, %f, %f, ",
                        accumMid[0],accumMid[1],accumMid[2],accumMid[3],accumMid[4],accumMid[5],accumMid[6],accumMid[7]);
#endif          
//           /* Completeness */
//           if (bnn) {
//           }
...
BITMAP *vote_n_openmp(Params *p, BITMAP *b,
                      ...
                      RegionMasks *amask, BITMAP *aweight, BITMAP *ainit, RegionMasks *region_masks, BITMAP *aconstraint, int mask_self_only) {
  init_openmp(p);
  //return weighted_vote_n<PATCH_W>(p, b, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint);
  ...
  mexPrintf("vote_n_openmp, amask: %p, bmask: %p, aweight: %p, bweight: %p, mask_self_only: %d\n", amask, bmask, aweight, bweight, mask_self_only);
//#pragma omp parallel for schedule(static, 16)
#pragma omp parallel
  {
#if USE_OPENMP
...
{
  int tiles = p->cores;
  mexPrintf("vote_n_cputiled, IS_SIMPLE=%d, ALLOW_AMASK=%d, mask_self_only=%d\n", IS_SIMPLE, ALLOW_AMASK, mask_self_only);
  ...
  mexPrintf("amask: %p, bmask: %p, aweight: %p, bweight: %p\n", amask, bmask, aweight, bweight);
  #pragma omp parallel num_threads(tiles)
  {
    ...
    for (int ay = ymin/*+ystep*/; ay < ymax; ay++ /*+=p->patch_w*/) {
      ...
      for (int ax = box.xmin; ax < box.xmax; ax++) {
        ...
        if (!IS_SIMPLE && aweight) { w *= ((float *) aweight->line[ay+PATCH_W/2])[ax+PATCH_W/2]; /*mexPrintf("aweight: %f\n", w);*/ }
					...
					BITMAP *ann_window, BITMAP *awinsize) 
{
  ...
  mexPrintf("in nn_n, masks are: %p %p %p, tiles=%d\n", amask, bmask, region_masks, tiles);
  Box box = get_abox(p, a, amask);
  int nn_iter = 0;
  ...
  mexPrintf("done nn_n, did %d iters, rs_max=%d\n", nn_iter, p->rs_max);
}
...
void nn_n_gpucpu(Params *p, BITMAP *a, BITMAP *b,
  ...
  mexPrintf("in nn_n_gpucpu, masks are: %p %p %p\n", amask, bmask, region_masks);
  Box box = get_abox(p, a, amask);
					...
					BITMAP *ann_window, BITMAP *awinsize) 
{
  ...
  mexPrintf("in nn_n_cputiled, masks are: %p %p %p, tiles=%d, rs_max=%d\n", amask, bmask, region_masks, tiles, p->rs_max);
  Box box = get_abox(p, a, amask);
  int nn_iter = 0;
  for (; nn_iter < p->nn_iters; nn_iter++) {
    unsigned int iter_seed = rand();
  ...
  mexPrintf("done nn_n_cputiled, %d iters, rs_max=%d\n", nn_iter, p->rs_max);
}
...
void nn_n_proponly(Params *p, BITMAP *a, BITMAP *b,
          ...
          int level, int em_iter, RecomposeParams *rp, int offset_iter, int update_type, RegionMasks *region_masks, int tiles) {
  if (tiles < 0) { tiles = p->cores; }
  mexPrintf("in nn_n_proponly, masks are: %p %p %p, tiles=%d\n", amask, bmask, region_masks, tiles);
  Box box = get_abox(p, a, amask);
  for (int nn_iter = 0; nn_iter < p->nn_iters; nn_iter++) {
    unsigned int iter_seed = rand();
...
void nn_n_fullrand(Params *p, BITMAP *a, BITMAP *b,
  ...
  mexPrintf("in nn_n_fullrand, masks are: %p %p %p, tiles=%d\n", amask, bmask, region_masks, tiles);
  init_gauss_table();
...
BITMAP *init_dist(Params *p, BITMAP *a, BITMAP *b, BITMAP *ann, BITMAP *bmask, RegionMasks *region_masks, RegionMasks *amask) {
  ...
  if (is_window(p)) {
    ...
    //mexPrintf("init_dist is unmasked\n");
...
void nn(Params *p, BITMAP *a, BITMAP *b,
  ...
  if (algo == ALGO_CPU) {
    if (is_window(p)) {
      mexPrintf("Running nn, using windowed and masked\n");
      ...
      mexPrintf("Running nn, using unmasked\n");
      ...
      mexPrintf("Running nn, using masked\n");
    ...
    if (is_window(p)) {
      mexPrintf("Running nn cputiled, using windowed and masked\n");
      ...
      //mexPrintf("Using unmasked\n");
      if (p->rs_max == 0) {
        mexPrintf("Running nn cputiled, using propagation only\n");
        ...
        mexPrintf("Running nn cputiled, no windows or masks\n");
      ...
      mexPrintf("Running nn cputiled, using masks\n");
...
/////////////////////////////////////////////////////////////////////////////////////////////
BITMAP *single_local_vote(Params *p, BITMAP *a, BITMAP *b, double *accum,
        BITMAP *bmask, BITMAP *bweight,
        double coherence_weight, double complete_weight,
        RegionMasks *amask, BITMAP *aweight, BITMAP *ainit, double *flattenedKernel,
        RegionMasks *region_masks, BITMAP *aconstraint, int mask_self_only, int factor, int cutoff, int radius) 
{
          BITMAP *ans = NULL;
          mexPrintf("SIMON: in nn.cpp:: single_local_vote - start. ADDED p->cores = 1;\n");
          p->cores = 1;
          if (p->vote_algo == VOTE_MEAN) 
          {
                    mexPrintf("$$$ voting mean, 1 core,   with patch width:%d\n",p->patch_w);
                    if      (p->patch_w == 1) { ans = single_local_vote_n<1>(p, a, b, accum, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, radius); }
                    else if (p->patch_w == 2) { ans = single_local_vote_n<2>(p, a, b, accum, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, radius); }
                    else if (p->patch_w == 3) { ans = single_local_vote_n<3>(p, a, b, accum, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, radius); }
                    else if (p->patch_w == 4) { ans = single_local_vote_n<4>(p, a, b, accum, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, radius); }
                    else if (p->patch_w == 5) { ans = single_local_vote_n<5>(p, a, b, accum, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, radius); }
                    else if (p->patch_w == 6) { ans = single_local_vote_n<6>(p, a, b, accum, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, radius); }
                    else if (p->patch_w == 7) { ans = single_local_vote_n<7>(p, a, b, accum, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, radius); }
                    else if (p->patch_w == 8) { ans = single_local_vote_n<8>(p, a, b, accum, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, radius); }
                    else if (p->patch_w == 9) { ans = single_local_vote_n<9>(p, a, b, accum, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, radius); }
                    else if (p->patch_w == 10) { ans = single_local_vote_n<10>(p, a, b, accum, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, radius); }
                    else if (p->patch_w == 11) { ans = single_local_vote_n<11>(p, a, b, accum, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, radius); }
                    else if (p->patch_w == 12) { ans = single_local_vote_n<12>(p, a, b, accum, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, radius); }
                    else if (p->patch_w == 13) { ans = single_local_vote_n<13>(p, a, b, accum, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, radius); }
                    else if (p->patch_w == 14) { ans = single_local_vote_n<14>(p, a, b, accum, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, radius); }
                    else if (p->patch_w == 15) { ans = single_local_vote_n<15>(p, a, b, accum, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, radius); }
                    else if (p->patch_w == 16) { ans = single_local_vote_n<16>(p, a, b, accum, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, radius); }
                    else if (p->patch_w == 17) { ans = single_local_vote_n<17>(p, a, b, accum, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, radius); }
                    else if (p->patch_w == 18) { ans = single_local_vote_n<18>(p, a, b, accum, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, radius); }
                    else if (p->patch_w == 19) { ans = single_local_vote_n<19>(p, a, b, accum, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, radius); }
                    else if (p->patch_w == 20) { ans = single_local_vote_n<20>(p, a, b, accum, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, radius); }
                    else if (p->patch_w == 21) { ans = single_local_vote_n<21>(p, a, b, accum, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, radius); }
                    else if (p->patch_w == 22) { ans = single_local_vote_n<22>(p, a, b, accum, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, radius); }
                    else if (p->patch_w == 23) { ans = single_local_vote_n<23>(p, a, b, accum, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, radius); }
                    else if (p->patch_w == 24) { ans = single_local_vote_n<24>(p, a, b, accum, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, radius); }
                    else if (p->patch_w == 25) { ans = single_local_vote_n<25>(p, a, b, accum, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, radius); }
                    else if (p->patch_w == 26) { ans = single_local_vote_n<26>(p, a, b, accum, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, radius); }
                    else if (p->patch_w == 27) { ans = single_local_vote_n<27>(p, a, b, accum, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, radius); }
                    else if (p->patch_w == 28) { ans = single_local_vote_n<28>(p, a, b, accum, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, radius); }
                    else if (p->patch_w == 29) { ans = single_local_vote_n<29>(p, a, b, accum, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, radius); }
                    else if (p->patch_w == 30) { ans = single_local_vote_n<30>(p, a, b, accum, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, radius); }
                    else if (p->patch_w == 31) { ans = single_local_vote_n<31>(p, a, b, accum, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, radius); }
                    else if (p->patch_w == 32) { ans = single_local_vote_n<32>(p, a, b, accum, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, radius); }
                    else { fprintf(stderr, "Patch size unsupported: %d\n", p->patch_w); exit(1); }                  
          }
          else 
          {
...
BITMAP *weighted_vote(Params *p, BITMAP *a, BITMAP *b, double *accum,
        BITMAP *ann, BITMAP *bnn,
        BITMAP *bmask, BITMAP *bweight,
        double coherence_weight, double complete_weight,
        RegionMasks *amask, BITMAP *aweight, BITMAP *ainit, double *flattenedKernels,
...
{
          BITMAP *ans = NULL;
          mexPrintf("SIMON: in nn.cpp:: vote - start. ADDED p->cores = 1;\n");
          p->cores = 1;
          if (p->vote_algo == VOTE_MEAN) 
          {
                    mexPrintf("voting mean, 1 core\n");
                    if      (p->patch_w == 1) { ans = weighted_vote_n<1>(p, a, b, accum, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff); }
                    else if (p->patch_w == 2) { ans = weighted_vote_n<2>(p, a, b, accum, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff); }
                    else if (p->patch_w == 3) { ans = weighted_vote_n<3>(p, a, b, accum, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff); }
                    else if (p->patch_w == 4) { ans = weighted_vote_n<4>(p, a, b, accum, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff); }
                    else if (p->patch_w == 5) { ans = weighted_vote_n<5>(p, a, b, accum, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff); }
                    else if (p->patch_w == 6) { ans = weighted_vote_n<6>(p, a, b, accum, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff); }
                    else if (p->patch_w == 7) { ans = weighted_vote_n<7>(p, a, b, accum, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff); }
                    else if (p->patch_w == 8) { ans = weighted_vote_n<8>(p, a, b, accum, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff); }
                    else if (p->patch_w == 9) { ans = weighted_vote_n<9>(p, a, b, accum, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff); }
                    else if (p->patch_w == 10) { ans = weighted_vote_n<10>(p, a, b, accum, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff); }
                    else if (p->patch_w == 11) { ans = weighted_vote_n<11>(p, a, b, accum, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff); }
                    else if (p->patch_w == 12) { ans = weighted_vote_n<12>(p, a, b, accum, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff); }
                    else if (p->patch_w == 13) { ans = weighted_vote_n<13>(p, a, b, accum, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff); }
                    else if (p->patch_w == 14) { ans = weighted_vote_n<14>(p, a, b, accum, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff); }
                    else if (p->patch_w == 15) { ans = weighted_vote_n<15>(p, a, b, accum, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff); }
                    else if (p->patch_w == 16) { ans = weighted_vote_n<16>(p, a, b, accum, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff); }
                    else if (p->patch_w == 17) { ans = weighted_vote_n<17>(p, a, b, accum, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff); }
                    else if (p->patch_w == 18) { ans = weighted_vote_n<18>(p, a, b, accum, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff); }
                    else if (p->patch_w == 19) { ans = weighted_vote_n<19>(p, a, b, accum, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff); }
                    else if (p->patch_w == 20) { ans = weighted_vote_n<20>(p, a, b, accum, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff); }
                    else if (p->patch_w == 21) { ans = weighted_vote_n<21>(p, a, b, accum, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff); }
                    else if (p->patch_w == 22) { ans = weighted_vote_n<22>(p, a, b, accum, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff); }
                    else if (p->patch_w == 23) { ans = weighted_vote_n<23>(p, a, b, accum, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff); }
                    else if (p->patch_w == 24) { ans = weighted_vote_n<24>(p, a, b, accum, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff); }
                    else if (p->patch_w == 25) { ans = weighted_vote_n<25>(p, a, b, accum, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff); }
                    else if (p->patch_w == 26) { ans = weighted_vote_n<26>(p, a, b, accum, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff); }
                    else if (p->patch_w == 27) { ans = weighted_vote_n<27>(p, a, b, accum, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff); }
                    else if (p->patch_w == 28) { ans = weighted_vote_n<28>(p, a, b, accum, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff); }
                    else if (p->patch_w == 29) { ans = weighted_vote_n<29>(p, a, b, accum, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff); }
                    else if (p->patch_w == 30) { ans = weighted_vote_n<30>(p, a, b, accum, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff); }
                    else if (p->patch_w == 31) { ans = weighted_vote_n<31>(p, a, b, accum, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff); }
                    else if (p->patch_w == 32) { ans = weighted_vote_n<32>(p, a, b, accum, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff); }
                    else { fprintf(stderr, "Patch size unsupported: %d\n", p->patch_w); exit(1); }                  
          }
          else 
          {
...
/////////////////////////////////////////////////////////////////////////////////////////////
BITMAP *weighted_KNN_vote(Params *p, int K_of_KNN, BITMAP *a, BITMAP *b, double *accum, BITMAP **a_knn, BITMAP *bmask, BITMAP *bweight,
						  double coherence_weight, double complete_weight,RegionMasks *amask, BITMAP *aweight, BITMAP *ainit, double *flattenedKernel,
						  RegionMasks *region_masks, BITMAP *aconstraint, int mask_self_only, int factor, int cutoff) 
{
          BITMAP *ans = NULL;
          mexPrintf("GUY: in nn.cpp:: weighted_KNN_vote - start. ADDED p->cores = 1;\n");
          p->cores = 1;
          if (p->vote_algo == VOTE_MEAN) 
          {
                    mexPrintf("$$$ voting mean, 1 core,   with patch width:%d\n",p->patch_w);
                    if      (p->patch_w == 1) { ans = weighted_KNN_vote_n<1>(p, a, b, accum, a_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, K_of_KNN); }
                    else if (p->patch_w == 2) { ans = weighted_KNN_vote_n<2>(p, a, b, accum, a_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, K_of_KNN); }
                    else if (p->patch_w == 3) { ans = weighted_KNN_vote_n<3>(p, a, b, accum, a_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, K_of_KNN); }
                    else if (p->patch_w == 4) { ans = weighted_KNN_vote_n<4>(p, a, b, accum, a_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, K_of_KNN); }
                    else if (p->patch_w == 5) { ans = weighted_KNN_vote_n<5>(p, a, b, accum, a_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, K_of_KNN); }
                    else if (p->patch_w == 6) { ans = weighted_KNN_vote_n<6>(p, a, b, accum, a_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, K_of_KNN); }
                    else if (p->patch_w == 7) { ans = weighted_KNN_vote_n<7>(p, a, b, accum, a_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, K_of_KNN); }
                    else if (p->patch_w == 8) { ans = weighted_KNN_vote_n<8>(p, a, b, accum, a_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, K_of_KNN); }
                    else if (p->patch_w == 9) { ans = weighted_KNN_vote_n<9>(p, a, b, accum, a_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, K_of_KNN); }
                    else if (p->patch_w == 10) { ans = weighted_KNN_vote_n<10>(p, a, b, accum, a_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, K_of_KNN); }
                    else if (p->patch_w == 11) { ans = weighted_KNN_vote_n<11>(p, a, b, accum, a_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, K_of_KNN); }
                    else if (p->patch_w == 12) { ans = weighted_KNN_vote_n<12>(p, a, b, accum, a_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, K_of_KNN); }
                    else if (p->patch_w == 13) { ans = weighted_KNN_vote_n<13>(p, a, b, accum, a_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, K_of_KNN); }
                    else if (p->patch_w == 14) { ans = weighted_KNN_vote_n<14>(p, a, b, accum, a_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, K_of_KNN); }
                    else if (p->patch_w == 15) { ans = weighted_KNN_vote_n<15>(p, a, b, accum, a_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, K_of_KNN); }
                    else if (p->patch_w == 16) { ans = weighted_KNN_vote_n<16>(p, a, b, accum, a_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, K_of_KNN); }
                    else if (p->patch_w == 17) { ans = weighted_KNN_vote_n<17>(p, a, b, accum, a_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, K_of_KNN); }
                    else if (p->patch_w == 18) { ans = weighted_KNN_vote_n<18>(p, a, b, accum, a_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, K_of_KNN); }
                    else if (p->patch_w == 19) { ans = weighted_KNN_vote_n<19>(p, a, b, accum, a_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, K_of_KNN); }
                    else if (p->patch_w == 20) { ans = weighted_KNN_vote_n<20>(p, a, b, accum, a_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, K_of_KNN); }
                    else if (p->patch_w == 21) { ans = weighted_KNN_vote_n<21>(p, a, b, accum, a_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, K_of_KNN); }
                    else if (p->patch_w == 22) { ans = weighted_KNN_vote_n<22>(p, a, b, accum, a_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, K_of_KNN); }
                    else if (p->patch_w == 23) { ans = weighted_KNN_vote_n<23>(p, a, b, accum, a_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, K_of_KNN); }
                    else if (p->patch_w == 24) { ans = weighted_KNN_vote_n<24>(p, a, b, accum, a_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, K_of_KNN); }
                    else if (p->patch_w == 25) { ans = weighted_KNN_vote_n<25>(p, a, b, accum, a_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, K_of_KNN); }
                    else if (p->patch_w == 26) { ans = weighted_KNN_vote_n<26>(p, a, b, accum, a_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, K_of_KNN); }
                    else if (p->patch_w == 27) { ans = weighted_KNN_vote_n<27>(p, a, b, accum, a_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, K_of_KNN); }
                    else if (p->patch_w == 28) { ans = weighted_KNN_vote_n<28>(p, a, b, accum, a_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, K_of_KNN); }
                    else if (p->patch_w == 29) { ans = weighted_KNN_vote_n<29>(p, a, b, accum, a_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, K_of_KNN); }
                    else if (p->patch_w == 30) { ans = weighted_KNN_vote_n<30>(p, a, b, accum, a_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, K_of_KNN); }
                    else if (p->patch_w == 31) { ans = weighted_KNN_vote_n<31>(p, a, b, accum, a_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, K_of_KNN); }
                    else if (p->patch_w == 32) { ans = weighted_KNN_vote_n<32>(p, a, b, accum, a_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernel, region_masks, aconstraint, mask_self_only, factor, cutoff, K_of_KNN); }
                    else { fprintf(stderr, "Patch size unsupported: %d\n", p->patch_w); exit(1); }                  
          }
          else 
          {
          ...
          mexPrintf("$$$$$$$$$$ weighted_KNN_vote ADDR is: %p\n", accum);
          mexPrintf("Guy: end\n");

  return ans;
}

BITMAP *weighted_KNN_dispersion_vote(Params *p, int K_of_KNN , BITMAP *a, BITMAP *b, double *accum,
        BITMAP **a_knn, BITMAP **b_knn,
        BITMAP *bmask, BITMAP *bweight,
        double coherence_weight, double complete_weight,
        RegionMasks *amask, BITMAP *aweight, BITMAP *ainit, double *flattenedKernels,
...
{
          BITMAP *ans = NULL;
          mexPrintf("GUY: in nn.cpp:: weighted_KNN_dispersion_vote, K = %u;\n",K_of_KNN);
          p->cores = 1;
          if (p->vote_algo == VOTE_MEAN) 
          {
                    mexPrintf("voting mean, 1 core\n");
                    if      (p->patch_w == 1) { ans = weighted_KNN_dispersion_vote_n<1>(p, a, b, accum, a_knn, b_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff , K_of_KNN); }
                    else if (p->patch_w == 2) { ans = weighted_KNN_dispersion_vote_n<2>(p, a, b, accum, a_knn, b_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff , K_of_KNN); }
                    else if (p->patch_w == 3) { ans = weighted_KNN_dispersion_vote_n<3>(p, a, b, accum, a_knn, b_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff , K_of_KNN); }
                    else if (p->patch_w == 4) { ans = weighted_KNN_dispersion_vote_n<4>(p, a, b, accum, a_knn, b_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff , K_of_KNN); }
                    else if (p->patch_w == 5) { ans = weighted_KNN_dispersion_vote_n<5>(p, a, b, accum, a_knn, b_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff , K_of_KNN); }
                    else if (p->patch_w == 6) { ans = weighted_KNN_dispersion_vote_n<6>(p, a, b, accum, a_knn, b_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff , K_of_KNN); }
                    else if (p->patch_w == 7) { ans = weighted_KNN_dispersion_vote_n<7>(p, a, b, accum, a_knn, b_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff , K_of_KNN); }
                    else if (p->patch_w == 8) { ans = weighted_KNN_dispersion_vote_n<8>(p, a, b, accum, a_knn, b_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff , K_of_KNN); }
                    else if (p->patch_w == 9) { ans = weighted_KNN_dispersion_vote_n<9>(p, a, b, accum, a_knn, b_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff , K_of_KNN); }
                    else if (p->patch_w == 10) { ans = weighted_KNN_dispersion_vote_n<10>(p, a, b, accum, a_knn, b_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff , K_of_KNN); }
                    else if (p->patch_w == 11) { ans = weighted_KNN_dispersion_vote_n<11>(p, a, b, accum, a_knn, b_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff , K_of_KNN); }
                    else if (p->patch_w == 12) { ans = weighted_KNN_dispersion_vote_n<12>(p, a, b, accum, a_knn, b_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff , K_of_KNN); }
                    else if (p->patch_w == 13) { ans = weighted_KNN_dispersion_vote_n<13>(p, a, b, accum, a_knn, b_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff , K_of_KNN); }
                    else if (p->patch_w == 14) { ans = weighted_KNN_dispersion_vote_n<14>(p, a, b, accum, a_knn, b_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff , K_of_KNN); }
                    else if (p->patch_w == 15) { ans = weighted_KNN_dispersion_vote_n<15>(p, a, b, accum, a_knn, b_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff , K_of_KNN); }
                    else if (p->patch_w == 16) { ans = weighted_KNN_dispersion_vote_n<16>(p, a, b, accum, a_knn, b_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff , K_of_KNN); }
                    else if (p->patch_w == 17) { ans = weighted_KNN_dispersion_vote_n<17>(p, a, b, accum, a_knn, b_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff , K_of_KNN); }
                    else if (p->patch_w == 18) { ans = weighted_KNN_dispersion_vote_n<18>(p, a, b, accum, a_knn, b_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff , K_of_KNN); }
                    else if (p->patch_w == 19) { ans = weighted_KNN_dispersion_vote_n<19>(p, a, b, accum, a_knn, b_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff , K_of_KNN); }
                    else if (p->patch_w == 20) { ans = weighted_KNN_dispersion_vote_n<20>(p, a, b, accum, a_knn, b_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff , K_of_KNN); }
                    else if (p->patch_w == 21) { ans = weighted_KNN_dispersion_vote_n<21>(p, a, b, accum, a_knn, b_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff , K_of_KNN); }
                    else if (p->patch_w == 22) { ans = weighted_KNN_dispersion_vote_n<22>(p, a, b, accum, a_knn, b_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff , K_of_KNN); }
                    else if (p->patch_w == 23) { ans = weighted_KNN_dispersion_vote_n<23>(p, a, b, accum, a_knn, b_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff , K_of_KNN); }
                    else if (p->patch_w == 24) { ans = weighted_KNN_dispersion_vote_n<24>(p, a, b, accum, a_knn, b_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff , K_of_KNN); }
                    else if (p->patch_w == 25) { ans = weighted_KNN_dispersion_vote_n<25>(p, a, b, accum, a_knn, b_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff , K_of_KNN); }
                    else if (p->patch_w == 26) { ans = weighted_KNN_dispersion_vote_n<26>(p, a, b, accum, a_knn, b_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff , K_of_KNN); }
                    else if (p->patch_w == 27) { ans = weighted_KNN_dispersion_vote_n<27>(p, a, b, accum, a_knn, b_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff , K_of_KNN); }
                    else if (p->patch_w == 28) { ans = weighted_KNN_dispersion_vote_n<28>(p, a, b, accum, a_knn, b_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff , K_of_KNN); }
                    else if (p->patch_w == 29) { ans = weighted_KNN_dispersion_vote_n<29>(p, a, b, accum, a_knn, b_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff , K_of_KNN); }
                    else if (p->patch_w == 30) { ans = weighted_KNN_dispersion_vote_n<30>(p, a, b, accum, a_knn, b_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff , K_of_KNN); }
                    else if (p->patch_w == 31) { ans = weighted_KNN_dispersion_vote_n<31>(p, a, b, accum, a_knn, b_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff , K_of_KNN); }
                    else if (p->patch_w == 32) { ans = weighted_KNN_dispersion_vote_n<32>(p, a, b, accum, a_knn, b_knn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, flattenedKernels, region_masks, aconstraint, mask_self_only, factor, cutoff , K_of_KNN); }
                    else { fprintf(stderr, "Patch size unsupported: %d\n", p->patch_w); exit(1); }                  
          }
          else 
          {
          ...
          mexPrintf("GUY: weighted_KNN_dispersion_vote end\n");
...
void check_dists(Params *p, BITMAP *a, BITMAP *b, BITMAP *ann, BITMAP *annd, int max_dist, RegionMasks *amask) {
  ...
  for (int y = box.ymin; y < box.ymax; y++) {
    for (int x = box.xmin; x < box.xmax; x++) {
      ...
      if (abs(d1 - d2)>max_dist) {
        mexPrintf("Distances not equal at (%d, %d) => (%d, %d): %d should be %d\n", x, y, xp, yp, d1, d2);
        for (int dy = -1; dy <= 1; dy++) {
          for (int dx = -1; dx <= 1; dx++) {
            if ((unsigned) (xp+dx) < (unsigned) (b->w-p->patch_w+1) &&
                (unsigned) (yp+dy) < (unsigned) (b->h-p->patch_w+1)) {
              mexPrintf("%08d ", patch_dist(p, a, x, y, b, xp+dx, yp+dy));    
            }
            mexPrintf("\n");
          }
        }
  ...
  if (max_err > 0) 
		mexPrintf("check_dists: max_err=%d, min_val=%d should be %d\n", max_err, min_val, min_valb);
}
